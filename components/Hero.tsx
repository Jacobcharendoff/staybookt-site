      ...
      <div className="max-w-5xl mx-auto w-full">
          ...
          <>
            <div className="text-center mb-14">
              ...
              <p className="text-lg sm:text-xl text-platinum-soft max-w-2xl mx-auto leading-relaxed">
                Type your URL. Pulse pulls live data, runs the analysis, and shows you the single stage of your business
                that's losing the most revenue right now. Then we fix it.
              </p>
            </div>
            ...
            <form onSubmit={onSubmit} className="max-w-2xl mx-auto">
              <div className="analyzer-input rounded-2xl p-2 flex items-center gap-2">
                <input
                  ...
                  placeholder="yourbusiness.com or business name + city"
                  className="flex-1 bg-transparent border-0 outline-none px-5 py-4 text-base sm:text-lg text-white placeholder:text-mute-dark"
                  autoFocus
                />
                <button
                  type="submit"
                  disabled={!input.trim()}
                  className="bg-elec hover:bg-elec-light text-ink text-sm sm:text-base font-bold px-5 sm:px-7 py-4 rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
                >
                  Run Pulse →
                </button>
              </div>
              <p className="text-center text-xs text-mute mt-4">
                No signup. No credit card. Real data from your real business.
              </p>
            </form>

            <div className="mt-20 grid grid-cols-3 gap-6 max-w-3xl mx-auto text-center">
              {[
                { label: 'Live signals', value: '14+' },
                { label: 'Avg analysis time', value: '~90s' },
        ...
        {phase === 'analyzing' && (
          <div className="max-w-2xl mx-auto text-center py-16">
            ...
            <h2 className="font-display text-3xl sm:text-4xl tracking-[-0.03em] leading-[1.05] mb-10">
              Pulling live signals on your business.
            </h2>
                ...
                >
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                      i < stageIdx
                        ? 'bg-hvac/20 text-hvac border border-hvac/40'
                        : i === stageIdx
                        ? 'bg-elec/20 text-elec border border-elec/60 animate-pulse'
                        : 'bg-ink border border-divider'
                    }`}
                  >
                    {i < stageIdx ? '✓' : ''}
                  </span>
                  <span className={`text-sm sm:text-base ${i === stageIdx ? 'text-white' : 'text-platinum-soft'}`}>
                    {s}…
                  </span>
                </div>
              ))}
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-ink-soft/40 border border-divider/60 rounded-2xl p-6 sm:p-8 mb-6">
              <div className="flex items-start gap-6 mb-7">
                <div className="relative w-28 h-28 flex-shrink-0">
                  <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                    ...
                    <circle
                      ...
                      stroke="var(--elec)"
                      strokeWidth="8"
                      strokeDasharray={`${(m.score / 100) * 263.9} 263.9`}
                      strokeLinecap="round"
                    />
                  </svg>
                ...
                <div className="flex-1">
                  <p className="text-[10px] tracking-[0.25em] uppercase font-bold text-mute-dark mb-2">
                    Pulse Score
                  </p>
                ...
                {m.metrics.map((mt) => (
                  <div
                    ...
                    className={`rounded-xl border p-4 ${
                      mt.status === 'critical'
                        ? 'border-leak/40 bg-leak/5'
                        : mt.status === 'low'
                        ? 'border-divider/60 bg-ink-soft/40'
                        : 'border-divider/60 bg-ink-soft/40'
                    }`}
                  >
                    <p className="text-[10px] tracking-[0.2em] uppercase text-mute-dark font-semibold mb-2">
                      {mt.label}
                    </p>
            ...
            <div className="bg-elec/5 border border-elec/20 rounded-2xl p-5 sm:p-6 mb-6">
              <p className="text-platinum text-sm sm:text-base">
                <span className="text-elec font-semibold">Next:</span> Scroll for the full operating cycle and the
                fix path. The CAPTURE leak is highlighted — that's what we'd ship first.
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a
                ...
                className="bg-elec hover:bg-elec-light text-ink text-sm font-bold px-6 py-3.5 rounded-xl transition-colors text-center"
              >
                Walk through it with us → 30 min
              </a>
              <button
                onClick={reset}
                className="text-sm font-semibold text-platinum-soft hover:text-white transition-colors px-6 py-3.5 border border-divider/60 hover:border-white/30 rounded-xl"
              >
